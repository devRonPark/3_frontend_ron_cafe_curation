...
// 데이터 유효성 검증
// 이메일 인증 모달 창 띄우고, 인증 모달 창에서 이벤트 핸들링
// 폼 제출했을 때 경고 메시지 띄워주거나 백엔드로 데이터 전송하는 거

// TODO function화 (findElements) 변수들을 object 화
...
const handleInputChange = (evt, name, regExp) => {
  const target = evt.target;
  const inputVal = target.value;
  let inputCheckBtn;
  switch (name) {
    case 'nickname':
      inputCheckBtn = nicknameCheckBtn;
      break;
    case 'email':
      inputCheckBtn = emailCheckBtn;
      break;
    default:
      break;
  }
...
const handleNicknameCheck = evt => {
  console.log('닉네임 중복 확인 버튼 클릭됨!!!');
  checkUsernameExist(nicknameInput.value);
  // 닉네임 중복 확인됨
  isNicknameConfirmed = true;
};
const handleEmailCheck = evt => {
  console.log('이메일 중복 확인 버튼 클릭됨!!!');
  checkEmailExist(emailInput.value);
  // 이메일 중복 확인됨
...
nicknameInput.addEventListener('input', evt =>
  handleInputChange(evt, 'nickname', /^[가-힣]{3,16}$/),
);
...
nicknameInputWrapper.addEventListener('click', e => {
...
emailInputWrapper.addEventListener('click', e => {
  const target = e.target;
  if (target.disabled === true) {
    // 인풋 변경할 지 여부 묻는 창 띄우기
    const isEditOrNot = confirm('변경하시겠습니까?');
...
// 이메일 인증 모달 창 열기
emailVerifyBtn.addEventListener('click', evt => {
...
// 인증번호 이메일로 전송 요청 기능
emailSendRequstBtn.addEventListener('click', evt => {
  console.log('전송 요청 버튼 클릭됨');
...
// 인증번호 이메일로 재전송 요청 기능
emailResendRequstBtn.addEventListener('click', evt => {
  console.log('재전송 요청 버튼 클릭됨');
  const emailValue = emailInput.value;
  // 타이머 요소 기본 값으로 설정
  timerForVerifyNumber.textContent = '10:00';
  // 이전에 활성화된 타이머가 있다면 강제 종료
...
// 인증번호 확인 기능
checkVerifyNumberBtn.addEventListener('click', evt => {
  console.log('인증번호 확인 버튼 클릭됨');
  // 백엔드에 인증번호 확인 요청을 보낸다.
  // 인증번호 아직 유효한 지 판단
  // 서버에 저장된 인증번호와 일치하는지 판단
  // 서버로부터 200 OK 와 함께 { isEmailVerificed: true } 데이터를 응답으로 받는다.
  // 200 OK 응답 받을 시,
  // 얼럿 창 띄우기 : alert('이메일 인증이 완료되었습니다.');
  // 얼럿 창의 확인 버튼을클릭하면,
  // 현재 활성화된 이메일 인증 모달 창 종료
  // 인증상태 텍스트 미인증에서 인증으로 변경
  // 인증 버튼은 disabled 상태가 된다.
  // isEmailVerified = true 가 된다.
  axios({
    method: 'post',
...
// 이메일 주소 수정 기능
emailEditBtn.addEventListener('click', evt => {
...
registerBtn.addEventListener('click', evt => {
  ...
  registerFormData.append('image_path', userProfileInput.files[0]);
  ...
  // Display the key/value pairs of register form data;
